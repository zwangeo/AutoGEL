...
class CompGCNBase(BaseModel):
	...
	def update_z(self, epoch):
		################################################################################################################
		# micro update
		if self.p.gcn_layer == 1:
			self.conv1.update_z_micro(epoch)
			self.Z_dict['agg'].append(self.conv1.Z_agg.cpu().tolist())
			self.Z_dict['combine'].append(self.conv1.Z_combine.cpu().tolist())
			self.Z_dict['comp'].append(self.conv1.Z_comp.cpu().tolist())

		if self.p.gcn_layer == 2:
			self.conv1.update_z_micro(epoch)
			self.conv2.update_z_micro(epoch)
			self.Z_dict['agg'].append(self.conv1.Z_agg.cpu().tolist() + self.conv2.Z_agg.cpu().tolist())
			self.Z_dict['combine'].append(self.conv1.Z_combine.cpu().tolist() + self.conv2.Z_combine.cpu().tolist())
			self.Z_dict['comp'].append(self.conv1.Z_comp.cpu().tolist() + self.conv2.Z_comp.cpu().tolist())

		################################################################################################################
		# macro update
		if self.p.macro_search:
			self.Z_layer_agg = self.get_Z(self.log_alpha_layer_agg, self.temperature, self.p)
	...
	def derive_arch(self):
		...
		for key in self.Z_dict.keys():
			self.searched_arch_z[key] = self.get_z_hard(self.Z_dict[key][self.best_epoch], self.p)
		...
		self.conv1.Z_agg = torch.tensor(self.conv1.Z_dict_micro['agg'][self.best_epoch], device=self.device)
		self.conv1.Z_combine = torch.tensor(self.conv1.Z_dict_micro['combine'][self.best_epoch], device=self.device)
		self.conv1.Z_comp = torch.tensor(self.conv1.Z_dict_micro['comp'][self.best_epoch], device=self.device)
		if self.p.gcn_layer == 2:
			self.conv2.Z_agg = torch.tensor(self.conv2.Z_dict_micro['agg'][self.best_epoch], device=self.device)
			self.conv2.Z_combine = torch.tensor(self.conv2.Z_dict_micro['combine'][self.best_epoch], device=self.device)
			self.conv2.Z_comp = torch.tensor(self.conv2.Z_dict_micro['comp'][self.best_epoch], device=self.device)

		if self.p.macro_search:
			self.Z_layer_agg = torch.tensor(self.Z_dict['layer_agg'][self.best_epoch], device=self.device)
	...
	def forward_base(self, sub, rel, drop1, drop2):
		...
		if self.p.macro_search:
			x = self.layer_agg_trans(self.Z_layer_agg, emb='x')
			r = self.layer_agg_trans(self.Z_layer_agg, emb='r')
		self.emb_list_x = []
		self.emb_list_r = []
