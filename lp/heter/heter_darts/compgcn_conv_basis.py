...
class CompGCNConvBasis(MessagePassing):
	...
	def forward(self, emb_list_r, x, edge_index, edge_type, rel_embed, first_layer=False):
		...
		in_res = self.propagate(self.Z_agg, self.in_index, x=x, edge_type=self.in_type, rel_embed=rel_embed,
								edge_norm=self.in_norm, mode='in')
		loop_res = self.propagate(self.Z_agg, self.loop_index, x=x, edge_type=self.loop_type, rel_embed=rel_embed,
								  edge_norm=None, mode='loop')
		out_res = self.propagate(self.Z_agg, self.out_index, x=x, edge_type=self.out_type, rel_embed=rel_embed,
