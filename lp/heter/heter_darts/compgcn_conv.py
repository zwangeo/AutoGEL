...
class CompGCNConv(MessagePassing):
	...
	def update_z_micro(self, epoch):
		...
		self.Z_comp = self.get_z(self.log_alpha_comp, self.temperature, self.args)
		...
		self.Z_dict_micro['agg'].append(self.Z_agg.cpu().tolist())
		self.Z_dict_micro['combine'].append(self.Z_combine.cpu().tolist())
		self.Z_dict_micro['comp'].append(self.Z_comp.cpu().tolist())
		self.Z_dict_micro = dict(self.Z_dict_micro)
	...
	def derive_arch_micro(self, best_epoch):
		...
		self.Z_agg = self.get_z_hard(self.Z_dict_micro['agg'][best_epoch], self.args)
		self.Z_combine = self.get_z_hard(self.Z_dict_micro['combine'][best_epoch], self.args)
		self.Z_comp = self.get_z_hard(self.Z_dict_micro['comp'][best_epoch], self.args)
	...
	def __init__(self, in_channels, out_channels, num_rels, act=lambda x: x, params=None):
		...
		self.Z_dict_micro = ddict(list)
		self.init_alpha_micro()
		# self.update_z_hard_micro(epoch=0)
		self.update_z_micro(epoch=0)
	...
	def forward(self, emb_list_r, x, edge_index, edge_type, rel_embed, first_layer=False):
		...
		in_res		= self.propagate(self.Z_agg, self.in_index,   x=x, edge_type=self.in_type,   rel_embed=rel_embed, edge_norm=self.in_norm, 	mode='in')
		loop_res	= self.propagate(self.Z_agg, self.loop_index, x=x, edge_type=self.loop_type, rel_embed=rel_embed, edge_norm=None, 		mode='loop')
		out_res		= self.propagate(self.Z_agg, self.out_index,  x=x, edge_type=self.out_type,  rel_embed=rel_embed, edge_norm=self.out_norm,	mode='out')
	...
	def combine_trans(self, in_res, loop_res, out_res):
		...
		out = torch.einsum('ij,jkl -> ikl', self.Z_combine, out).squeeze(0)
		return out
	...
	def rel_transform(self, ent_embed, rel_embed):
		...
		trans_embed = torch.einsum('ij,jkl -> ikl', self.Z_comp, trans_embed).squeeze(0)
		return trans_embed
