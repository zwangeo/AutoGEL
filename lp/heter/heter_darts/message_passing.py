...
def scatter_(z_agg, agg_list, src, index, dim_size=None):
	...
	out = torch.einsum('ij,jkl -> ikl', z_agg, out).squeeze(0)
...
class MessagePassing(torch.nn.Module):
	...
	def propagate(self, z_agg, edge_index, **kwargs):
		...
		out = scatter_(z_agg, self.search_space['agg'], out, edge_index[0], dim_size=size)
