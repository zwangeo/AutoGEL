...
# def resplit_dataset(dataset, args, logger):
#     edge_index = dataset[0].edge_index
#     x = dataset[0].x
#     y = dataset[0].y
#     train_mask, val_mask, test_mask = reset_mask(dataset, args, logger, stratify=None)
#
#     dataset = Data(edge_index=edge_index, test_mask=test_mask, train_mask=train_mask, val_mask=val_mask, x=x, y=y)
#     return dataset

def helper_ppi(args):
    train_dataset = PPI(root='./data/' + args.task + '/' + args.dataset, split='train')
    val_dataset = PPI(root='./data/' + args.task + '/' + args.dataset, split='val')
    test_dataset = PPI(root='./data/' + args.task + '/' + args.dataset, split='test')

    in_features = train_dataset.num_node_features
    ...
    return in_features, out_features, train_mask, val_mask, test_mask, train_loader, val_loader, test_loader



def reset_mask(dataset, args, logger, stratify=None):
    if args.task == 'graph':
        train_mask, val_mask, test_mask = None, None, None
        return train_mask, val_mask, test_mask

    elif args.task == 'node':
        n_samples = dataset[0].x.shape[0]
        train_indices, test_indices = train_test_split(list(range(n_samples)), test_size=(args.val_ratio + args.test_ratio),
                                                       stratify=stratify)
        val_indices, test_indices = train_test_split(test_indices, test_size=args.test_ratio / (args.val_ratio + args.test_ratio),
                                                     stratify=stratify)
        logger.info('Train size :{}, val size: {}, test size: {}, val ratio: {}, test ratio: {}'.format(len(train_indices),
                                                                                                        len(val_indices),
                                                                                                        len(test_indices),
                                                                                                        args.val_ratio,
                                                                                                        args.test_ratio))
...
def get_data(args, logger):
    ...
    in_features = max(dataset.num_node_features, 1)
    out_features = dataset.num_classes
    logger.info('Input dimensions :{}, output dimensions :{}'.format(in_features, out_features))
...
def get_loader(dataset, args):
    if args.task == 'graph':
        ...
        train_loader = DataLoader(dataset[:int(data_size * args.train_ratio)], batch_size=args.bs, shuffle=True)
        val_loader = DataLoader(dataset[int(data_size * args.train_ratio):int(data_size * (args.train_ratio+args.val_ratio))], batch_size=args.bs, shuffle=True)
        test_loader = DataLoader(dataset[int(data_size * (args.train_ratio+args.val_ratio)):], batch_size=args.bs, shuffle=True)
        return train_loader, val_loader, test_loader

    elif args.task == 'node':
        train_loader = val_loader = test_loader = DataLoader(dataset, batch_size=args.bs, shuffle=True)
        return train_loader, val_loader, test_loader
...
def get_model(layers, in_features, out_features, args, logger):
