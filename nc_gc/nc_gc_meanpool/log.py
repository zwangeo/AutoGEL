...
def save_performance_result(args, logger, model):
    ...
    model_name = '-'.join([args.model, str(args.layers)])
    best_metric_search = model.best_metric_search
    best_metric_retrain = model.best_metric_retrain
    ...
    line = '\t'.join([dataset, model_name, str(seed), 'search', str(round(best_metric_search, 4)), 'retrain', str(round(best_metric_retrain, 4)), log_name, server]) + '\n'
    with open(summary_file, 'a') as f:
        f.write(line)  # WARNING: process unsafe!


# def save_performance_result(args, logger, metrics, mode):
#     # mode in ['search', 'retrain']
#     summary_file = args.summary_file
#     if summary_file != 'test':
#         summary_file = os.path.join(args.log_dir, summary_file)
...
#     server = socket.gethostname()
#     #     line = '\t'.join([dataset, model_name, str(seed), str(round(val_metric, 4)), str(round(no_val_metric, 4)), log_name, server]) + '\n'
#     line = '\t'.join([dataset, model_name, str(seed), mode, str(round(metrics, 4)), log_name, server]) + '\n'
#     with open(summary_file, 'a') as f:
#         f.write(line)  # WARNING: process unsafe!
